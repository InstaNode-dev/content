...
- [Browser job queue with retries](/use-cases/browser-job-queue-with-retries) — execution primitive each cron tick fans out onto
- [Scraped product-price history](/use-cases/scraped-product-price-history) — longitudinal storage for what these scrapers extract
- [Overnight dossier fleet](/use-cases/overnight-dossier-fleet) — another scheduled background fleet pattern with Mongo+S3-compatible storage outputs
