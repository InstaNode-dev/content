...
I want to: an analyst agent splits a 200-cell notebook across 30 Pyodide workers, each writing intermediate dataframes to S3-compatible storage so the planner can pull only the ones it needs.

Write a complete runnable script (bash + whatever language fits) that:
- Provisions the services I need (S3-compatible storage + NATS JetStream) from instanode.dev
...
NATS JetStream is queue + durable log in one — workers can rejoin mid-run and replay missed cell assignments. S3-compatible storage is S3-API so the same `boto3` / `fsspec` code works in browser Pyodide and Node, and the bucket is provisioned in under a second with zero account setup.
